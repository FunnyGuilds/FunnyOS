...
namespace FunnyOS::Stdlib::Algorithm {
    ...
    template <typename T> constexpr const T& Min(const T& a, const T& b) noexcept(noexcept(a < b)) {
        return a < b ? a : b;
    }

    template <typename T> constexpr const T& Max(const T& a, const T& b) noexcept(noexcept(a > b)) {
        return a > b ? a : b;
    }
}  // namespace FunnyOS::Stdlib::Algorithm
