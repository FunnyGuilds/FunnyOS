...
namespace FunnyOS::Stdlib::Algorithm {
     ...
     */
    template <typename T>[[nodiscard]] constexpr const T& Min(const T& a, const T& b) noexcept(noexcept(a < b));
     ...
     */
    template <typename T>[[nodiscard]] constexpr const T& Max(const T& a, const T& b) noexcept(noexcept(a > b));
