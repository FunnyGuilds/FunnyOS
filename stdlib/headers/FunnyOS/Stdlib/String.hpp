...
namespace FunnyOS::Stdlib::String {
    ...
    class BasicString {
       ...
       public:
        TRIVIALLY_COPYABLE(BasicString);
        TRIVIALLY_MOVEABLE(BasicString);

        BasicString(const char* data);

        BasicString(const Memory::SizedBuffer<T>& buffer);
       ...
       public:
         ...
         */
        void Append(const BasicString<T>& value);


        Stdlib::Vector<T>& AsCharacterVector();

        const Stdlib::Vector<T>& AsCharacterVector() const;
    ...
    using DynamicString = BasicString<char>;
