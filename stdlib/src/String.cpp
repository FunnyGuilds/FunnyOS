...
namespace FunnyOS::Stdlib::String {
    ...
    template <typename T>
    BasicString<T>::BasicString(const char* data) {
        m_data.Insert(0, data, Length(data));
    }

    template <typename T>
    BasicString<T>::BasicString(const Memory::SizedBuffer<T>& buffer) {
        m_data.Insert(0, buffer.Data, buffer.Size);
    }

    template <typename T>
    size_t BasicString<T>::Size() const noexcept {
        return m_data.Size();
    }

    template <typename T>
    void BasicString<T>::ShrinkToSize() {
        m_data.ShrinkToSize();
    }

    template <typename T>
    void BasicString<T>::EnsureCapacity(size_t num) {
    ...
    template <typename T>
    void BasicString<T>::Append(const BasicString<T>& value) {
        m_data.Insert(m_data.Size(), value.Begin(), value.Size());
    }

    template <typename T>
    Stdlib::Vector<T>& BasicString<T>::AsCharacterVector() {
        return m_data;
    }

    template <typename T>
    const Stdlib::Vector<T>& BasicString<T>::AsCharacterVector() const {
        return m_data;
    }

    template <typename T>
    typename BasicString<T>::Iterator BasicString<T>::Begin() noexcept {
        return m_data.Begin();
    }

    template <typename T>
    typename BasicString<T>::Iterator BasicString<T>::End() noexcept {
        return m_data.End();
    }

    template <typename T>
    typename BasicString<T>::ConstIterator BasicString<T>::Begin() const noexcept {
        return m_data.Begin();
    }

    template <typename T>
    typename BasicString<T>::ConstIterator BasicString<T>::End() const noexcept {
        return m_data.End();
    }
