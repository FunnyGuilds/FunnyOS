...
namespace FunnyOS::Bootloader32 {
    ...
    namespace {
        constexpr sectornumber_t DEFAULT_SECTOR_SIZE = 0x200;
        constexpr sectornumber_t EDD_MAX_READ        = 0x7F;
    ...
    DriveInterface::DriveInterface(drive_indentification_t drive) : m_drive(drive) {
        Query();
    }

    drive_indentification_t DriveInterface::GetDriveIdentification() const {
        return m_drive;
    }

    sectornumber_t DriveInterface::GetTotalSectorCount() const {
    ...
    void DriveInterface::ReadSectors(
        sectornumber_t sector, sectornumber_t count, Memory::SizedBuffer<uint8_t>& buffer) {
        if (buffer.Size < count * GetSectorSize()) {
            F_ERROR_WITH_MESSAGE(DriveInterfaceException, "buffer to small to hold all sectors");
        }

        sectornumber_t currentOffset = 0;
        sectornumber_t leftToRead    = count;

        if (m_supportsFlat64Addresses) {
            while (leftToRead > 0) {
                const sectornumber_t currentReadSize = Min(leftToRead, EDD_MAX_READ);
        ...
        const sectornumber_t bufferSize = GetRealModeBuffer().Size / m_sectorSize;
        ...
        if (m_hasExtendedDiskAccess) {
            ...
            while (leftToRead > 0) {
                const sectornumber_t currentReadSize = Min(leftToRead, EDD_MAX_READ, bufferSize);
        ...
        while (leftToRead > 0) {
            const sectornumber_t currentLba = sector + currentOffset;
            ...
            const sectornumber_t leftSectorsOnTrack = m_sectorsPerTrack - sector + 1;
            const sectornumber_t currentReadSize    = Min(leftToRead, leftSectorsOnTrack, bufferSize);
